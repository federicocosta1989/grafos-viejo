...
pass_ = 'Lw8yNivJxfCF'
ver = '2018-02-16'
wks_id = '1825639a-2ccf-45ca-af2f-dbae43426489'
...
contexto={"edad": ""}
contexto={"sexo": ""}
contexto={"estado_civil": ""}
contexto={"lugar_nacimiento": ""}
contexto={"comida": ""}
contexto={"cantidad_hijos": ""}
contexto={"profesion": ""}
...
#            print(json.dumps(response, indent=2))
            ...
            if pregunta in preg_realizadas:
                print(pregunta + " esta dentro de las preguntas realizadas asi que la borro del context")
                response["context"]["pregunta"] = ""
#                print(json.dumps(response, indent=2))
                
            respuesta = response["context"][pregunta.lower()]
            print("la respuesta a " + pregunta + " es: ")
            print(respuesta)

            
            if response["output"]["nodes_visited"][-1] == "node_7_1520016504617":
                print("entro en el if")
                print("la mejor pregunta es: ")
                print(elegir_mejor_pregunta())
                response["context"]["pregunta"] = elegir_mejor_pregunta()
                preg_realizadas.append(elegir_mejor_pregunta())
                
            if (pregunta in response["context"]["pregunta"] and response["context"][pregunta.lower()] != ""):
                print("entro en el segundo if")
                respuesta = response["context"][pregunta.lower()]
                print("la respuesta a " + pregunta + " es:")
                print(respuesta)
#                print(json.dumps(response,indent=2))
                borrar_nodos(pregunta,respuesta)
                response["context"][pregunta.lower()]=""
#                print(json.dumps(response,indent=2))
                print(status())
#                salir = True
