...
#            print(json.dumps(response, indent=2))
            ...
            if response["output"]["nodes_visited"][-1] == "node_8_1519919216035" and len(preg_realizadas) == 0:
                
                if ("edad" in response["context"]):
                    borrar_nodos("Edad",response["context"]["edad"])
                if ("sexo" in response["context"]):
                    borrar_nodos("Sexo",response["context"]["sexo"])
                if ("estado_civil" in response["context"]):
                    borrar_nodos("Estado_Civil",response["context"]["estado_civil"])
                if ("cantidad_hijos" in response["context"]):
                    borrar_nodos("Cantidad_Hijos",response["context"]["cantidad_hijos"])
                if ("lugar_nacimiento" in response["context"]):
                    borrar_nodos("Lugar_Nacimiento",response["context"]["lugar_nacimiento"])
                if ("comida" in response["context"]):
                    borrar_nodos("Comida",response["context"]["comida"])
                if ("deporte" in response["context"]):
                    borrar_nodos("Deporte",response["context"]["deporte"])
                if ("profesion" in response["context"]):
                    borrar_nodos("Profesion",response["context"]["profesion"])
